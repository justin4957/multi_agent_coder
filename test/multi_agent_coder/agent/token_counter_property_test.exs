defmodule MultiAgentCoder.Agent.TokenCounterPropertyTest do
  ...
  describe "Token counting properties" do
    ...
    property "longer text has more tokens" do
      check all short <- string(:ascii, min_length: 1, max_length: 50),
                long <- string(:ascii, min_length: 100, max_length: 500) do
    ...
    property "concatenated text has approximately sum of tokens" do
      check all text1 <- string(:ascii, min_length: 10, max_length: 100),
                text2 <- string(:ascii, min_length: 10, max_length: 100) do
        ...
        margin = div(expected_sum, 5)  # 20% margin
    ...
    property "repeated text has more tokens than single" do
      check all text <- string(:ascii, min_length: 10, max_length: 50),
                repetitions <- integer(2..5) do
  ...
  describe "Cost calculation properties" do
    ...
    property "more tokens means higher cost" do
      check all tokens1 <- integer(10..1000),
                tokens2 <- integer(1001..10_000) do
    ...
    property "zero tokens means zero cost" do
      check all provider <- member_of([:openai, :anthropic, :deepseek, :perplexity, :local]),
                model <- string(:alphanumeric, min_length: 1, max_length: 20) do
    ...
    property "cost scales linearly with tokens" do
      check all base_tokens <- integer(100..1000) do
        ...
        double_cost = TokenCounter.calculate_cost(:openai, "gpt-4", base_tokens * 2, base_tokens * 2)
  ...
  describe "Usage summary properties" do
    property "usage summary contains all required fields" do
      check all provider <- member_of([:openai, :anthropic, :deepseek, :perplexity]),
                model <- string(:alphanumeric, min_length: 1, max_length: 20),
                input <- string(:ascii, min_length: 1, max_length: 100),
                output <- string(:ascii, min_length: 1, max_length: 100) do
    ...
    property "formatted cost matches calculated cost" do
      check all provider <- member_of([:openai, :anthropic, :deepseek, :perplexity]),
                ...
                output <- string(:ascii, min_length: 10, max_length: 50) do
        usage = TokenCounter.create_usage_summary(provider, model, input, output)
